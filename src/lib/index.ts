...
import type { Configuration, ParsedFormConfiguration, ParsedFormConfigurationButton, ParsedFormConfigurationField } from './types'
    ...
    return Object.entries( config ).map( 
        ([ formName, formConfiguration ]) =>
        {
            return {
                ...
                fields: Object.entries( formConfiguration.fields || {} ).map(
                    ([ name, fieldConfiguration ]) => {
                        const field = {
                            ...
                            label: fieldConfiguration.label === undefined ? fieldNameToLabelConverter(name) : fieldConfiguration.label !== null ? fieldConfiguration.label : null,
                ...
                buttons: Object.entries( formConfiguration.buttons || {} ).map(
                    ([ name, buttonConfiguration ]) => {
                        return{
                            ...
                            label: buttonConfiguration.label === undefined ? fieldNameToLabelConverter(name) : buttonConfiguration.label !== null ? buttonConfiguration.label : null,
...
export function create( configuration: Configuration )
{
    ...
    return new Proxy({}, {
            ...
            get( target: { [key: string | symbol]: any }, prop ){
                ...
                if( prop === 'createActions' ) return (actions: Actions) => createActions( parsedFormConfigurations, actions )
                ...
                if( typeof Form === 'object' )  return new Proxy( Form, {
                        // eslint-disable-next-line @typescript-eslint/no-explicit-any
                        get: ( target: { [key: string | symbol]: any }, prop ) =>
                        {
                            // eslint-disable-next-line @typescript-eslint/no-explicit-any
                            if (prop === 'render') return ( props: any, { $$slots = {}, context = new Map() } = {} ) => {
                                return target.render({ form: parsedFormConfiguration, ...props }, { $$slots, context })
                            }
                            
                            // eslint-disable-next-line @typescript-eslint/no-explicit-any
                            if (prop === '$$render') return ( result: any, props: any, bindings: any, slots: any, context: any ) => {
                                return target.$$render( result, { form: parsedFormConfiguration, ...props }, bindings, slots, context )
                            }
                ...
                return class extends Form {
                    constructor( options: unknown ) {
                        super({ ...options as any, props: { form: parsedFormConfiguration, ...((options as any)?.props ?? {}) } })
                    }
                }                    
            }
        }
 ...
 */
export function writeFieldErrors( field: ParsedFormConfigurationField, errors: any, formData: FormData | undefined = undefined )
{
    ...
    for ( const [validation, condition] of Object.entries( field.validate ) )
    {
        ...
        || ( validation === 'minLowercase' && value?.replace( regexes.uppercases, '' ).length < condition )
        || ( validation === 'maxLowercase' && value?.replace( regexes.uppercases, '' ).length > condition )
        || ( validation === 'hasUppercase' && !regexes.uppercase.test( value ) )
        || ( validation === 'minUppercase' && value?.replace( regexes.lowercases, '' ).length < condition )
        || ( validation === 'maxUppercase' && value?.replace( regexes.lowercases, '' ).length > condition )
        ...
        ) _errors.push( field.messages?.[ validation ]?.replaceAll('%required_amount%', condition + '') || 'Missing error message for ' + validation) // || defaultMessages.get(validation, condition, field )
    }
 ...
 */
export function createActions( parsedFormConfigurations: ParsedFormConfiguration[], actions: { [form: string]: (event: RequestEvent, form: any) => any })
{
 ...
 */
export function getFormElementAttributes(props: { [x: string]: any }, parsedFormConfiguration: ParsedFormConfiguration) : object
{
