...
export const actions = forms.createActions(
    {
        login: async ( event, { formData }) =>
        {
            ...
            if ( email === 'elon@twitter.com') throw fail(400, { email: "Only if you give me a free space ride I will let you in." }) 
...
```svelte
<style>
    :global(form[name="login"] label) { font-weight: bold }
</style>
```
...
export const actions = forms.createActions(
    {
        ...
        {
            if ( blacklist.includes( form.data.get( 'email' ) ) ) throw fail( 400, { email: 'Email is blacklisted. Please use a different one.' })
